...
void load_image() {
	pgmread("/content/image256x256.pgm", (void *)image, WIDTH, HEIGHT);
}

void save_image() {
	pgmwrite("image-outputl256x256.pgm", (void *)final, WIDTH, HEIGHT);
}
...
void call_kernel() {
   ...
   // Print the number of threads utilized
    int numThreads = threads.x * threads.y * blocks.x * blocks.y;
